...
//
// It generated by `prost`, for more detailed info, plz visit:
//
// https://crates.io/crates/prost
//
pub mod echo {
    //
    // Paramter `echo` is the package name defined in your proto file!!!
    //
    // After that, this module includes:
    //
    //  `Echo`: Generated trait containing gRPC methods that should be
    //          implemented for using with `EchoServer`.
    //
    //  `EchoServer`: gPRC server that implements the `Echo` trait.
    //
    //  `EchoRequest` and `EchoResponse`: Messages based on your proto file.
    //
